...
const Team: FC = (props) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const response = await fetch('/api/cities/', authRequest('GET'));
    ...
    const onSubmit = (ev: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!cities.map(item => item.cityName).includes(team.city.cityName)) {
            (async () => {
                const res = await fetch('/api/cities/add', authRequest('POST', { cityName: team.city.cityName }));
        ...
        (async () => {
            const promise = new Promise(async (resolve, reject) => {
                try {
                    ...
                    await fetch('/api/teams/add', authRequest('POST', { ...team, logoPath: res }));
                    resolve(null)
                    handlePopup(null);
                } catch (err) {
                    reject(err)
