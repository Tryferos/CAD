    ...
    (async () => {
      ...
      const lengthedChampData = await Promise.all(
        champData.map(async (vChampionship, i) => {
          const teamResponse = await fetch(`${process.env.NODE_ENV == 'development' ? `http://localhost:${process.env.REACT_APP_SERVER_PORT}` : ''}/api/teams/championship/${vChampionship.id}`)
          const teamData = await teamResponse.json() as AdminTeam[];
          const lengthedTeamData = await Promise.all
            (
              teamData.map(async vTeam => {
                const playersResponse = await fetch(`${process.env.NODE_ENV == 'development' ? `http://localhost:${process.env.REACT_APP_SERVER_PORT}` : ''}/api/players/team/${vTeam.id}`)
