...
export default function Matches() {
    ...
    const handleDraw = () => {
        (async () => {
            const res = await fetch(`http://localhost:3309/api/championships/createDraw/${tourid}?date=${formatDate()}`, authRequest('POST'))
        ...
        for (let i = 0; i < 5; i++) {
            try {
                ...
                const quarterRes = await fetch(
                    `http://localhost:3309/api/teamScorePerQuarters/teamScorePerQuarter?quarter=${quarterType}&gameId=${match.id}&roundId=${match.round_id}&championshipId=${tourid}&teamId=${team.id}`)
    ...
    return (
        <li className={`h-[200px] min-w-[300px] hover:shadow-box hover:scale-[1.02] transition-[transform,shadow] items-center flex bg-slate-100 cursor-pointer outline outline-1 outline-slate-200 px-0 py-8 relative rounded-md ${className}`}>
            ...
            </div>
            {
                (user &&
                    <div
                        onClick={() => handlePopup(PopupType.score, `${match.homeTeam.teamName} εναντίον ${match.awayTeam.teamName}, Γύρος ${match.round}`, { match: match, tourid: tourid })}
